<div class="p-4">
    ...
    <div class="flex items-center justify-between mb-4">
        ...
        @if(auth()->check() && auth()->user()->isAdmin())
            <button wire:click.prevent="$emit('openSupplierForm')" class="btn-primary inline-flex items-center gap-2">
        ...
        @foreach($suppliers as $supplier)
            <div class="border p-3 rounded">
                ...
                <div class="mt-3 flex space-x-2">
                    @if(auth()->check() && auth()->user()->isAdmin())
                        <button wire:click.prevent="$emit('openSupplierForm', {{ $supplier->id }})" class="px-2 py-1 bg-yellow-500 text-white rounded inline-flex items-center gap-1">
